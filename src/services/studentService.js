...
export const retrieveTeacherIdsByEmails = (emails) => Teachers.findAll({
  where: {
    email: {
      [Sequelize.Op.in]: emails,
    },
  },
}).then((teachers) => teachers.map((x) => x.dataValues.id));

export const retreiveStudentsEmails = (ids) => Students.findAll({
  where: {
    id: {
      [Sequelize.Op.in]: ids,
    },
...
const retrieveCommonStudentsIdsByTeachersIds = (ids) => TeachersToStudents.findAll({
  where: {
    teacher_id: {
      [Sequelize.Op.or]: ids,
    },
  },
}).then((teacherStudentMapping) => teacherStudentMapping.map((x) => x.dataValues.student_id));

const suspendStudentByEmail = (email) => Students.update({
  suspended: 1,
}, {
  where: {
    email,
...
export const suspendStudent = async (email) => await suspendStudentByEmail(email);
